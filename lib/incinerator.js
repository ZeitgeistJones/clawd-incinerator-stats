...
async function scanRange(fromBlock, toBlock) {
  ...
  async function worker() {
    while (cursor < ranges.length) {
      ...
      try {
        results[idx] = await fetchChunk(f, t);
      } catch (e) {
        // Never silently skip a range — that advances scannedTo past missing burns.
        failures.push({ from: f, to: t, error: e.message || String(e) });
        results[idx] = null;
      }
    }
  }
  ...
  if (failures.length) {
    const sample = failures.slice(0, 3).map((f) => `${f.from}-${f.to}: ${f.error}`).join("; ");
    throw new Error(`${failures.length} of ${ranges.length} block ranges failed (${sample})`);
  }
